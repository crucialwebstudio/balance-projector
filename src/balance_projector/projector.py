            ...
            if account_spec['scheduled_transactions']:
                for trans_id, trans in account_spec['scheduled_transactions'].items():
                    ...
                    if type(amount) == dict:
                        ...
                        transfer = None if trans['transfer'] is None \
                            else Transfer(direction=trans['transfer']['direction'],
                                          account_id=trans['transfer']['account_id'])
                        st = ScheduledTransaction(transaction_id=trans_id, account_id=account_id,
                                                  name=trans['name'],
                                                  amount=amount, type=trans['type'],
                                                  date_spec=DateSpec.from_spec(trans['date_spec']),
                                                  transfer=transfer)
                        """
